    ...
    if (isArray(templateApiArray)) {
      ...
      templateApiArray.forEach((apiItem, idx) => {
        ...
        if (isArray(apiItem)) {
          ...
          // { replicate: nodeNum }
          // { replicate: nodeNum, prefix: '01_' }
          const { replicate } = apiItem;
          const { prefix } = apiItem;
          if (Number.isFinite(replicate)) {
            if (arrayIndexPopulated(this.contents, replicate)) {
              if (isString(prefix) || prefix == null) {
                const thePrefix = prefix || '';
                const apiNode = this.contents[replicate];
                ...
                apiToCopy.forEach((constOrParam) => {
                  templateApiArr2ndPass.push([`${thePrefix}${constOrParam}`, replicate, constOrParam]);
                });
              } else {
