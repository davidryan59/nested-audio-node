...
class NestedAudioNode {
  constructor(inputData = {}) {
    ...
    this.logger.info({ text: `${tag}:  created NestedAudioNode ${type}:` });
  ...
  initialise({
    ...
    if (isArray(templateApiArray)) {
      ...
      const templateApiArr2ndPass = [...templateApiArray];
      templateApiArray.forEach((apiItem, idx) => {
        const c5tag = `${tag}-A.${idx}`;
        if (isObject(apiItem) && !isArray(apiItem)) { // Shouldn't need the second condition...
          if (apiItem.replicate) {
            if (arrayIndexPopulated(this.contents, apiItem.replicate)) {
              if (isString(apiItem.prefix) || apiItem.prefix == null) {
                this.logger.debug({ text: `${c5tag}:  Processing API replication object ${disp(apiItem)}...` });
                const thePrefix = apiItem.prefix || '';


                // Need to insert several API items here...
                this.logger.warn({ text: `${c5tag}:  NOT YET IMPLEMENTED Processing API replication object ${disp(apiItem)}...` });
              } else {
                this.logger.warn({ text: `${c5tag}:  param setup object ${disp(apiItem)} has invalid prefix string` });
              }
            } else {
              this.logger.warn({ text: `${c5tag}:  param setup object ${disp(apiItem)} has invalid replicate reference` });
            }
          } else {
